...
class KafkaLogReader {
    ...
    async heatCheck(){
        ...
        const tpTimeIndexFile = path.join(this.logDir, "test-topic-0/00000000000000031482.timeindex");
        ...
        const tpTimeIndexData = await fs.readFileAsync(tpTimeIndexFile);
        const tpLogData = await fs.readFileAsync(tpLogFile);

        debug("index", tpIndexData.length, "timeindex", tpTimeIndexData.length, "log", tpLogData.length);

        debug(tpIndexData);
        debug(tpTimeIndexData);
        debug(tpLogData);
    }
}
